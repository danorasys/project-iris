...
function Key({ content, accessibleLabel, onSelect, variant = "digit", disabled = false, compact = false }: KeyProps) {
  ...
  return (
    <button
      ref={ref}
      type="button"
      className={`${styles.key} ${styles[variant]} ${compact ? styles.keyCompact : ""} ${focused ? styles.focused : ""}`}
    ...
    <div className={`${styles.container} ${compact ? styles.containerCompact : ""}`}>
      <div className={styles.display} aria-live="polite">
        {Array.from({ length: maxLength }).map((_, i) => (
          <span
            key={i}
            className={`${styles.displayCell} ${compact ? styles.displayCellCompact : ""} ${i < value.length ? styles.displayCellFilled : ""}`}
          >
      ...
      <div className={`${styles.grid} ${compact ? styles.gridCompact : ""}`}>
        {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map((d) => (
          <Key key={d} content={d} accessibleLabel={`Dígito ${d}`} onSelect={() => addDigit(d)} compact={compact} />
        ))}
        <Key
          ...
          compact={compact}
        />
        <Key content="0" accessibleLabel="Dígito 0" onSelect={() => addDigit("0")} compact={compact} />
        <Key
