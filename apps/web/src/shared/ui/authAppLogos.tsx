...
/**
 * Authy's actual app icon (the white swirl on a red square), reproduced as a
 * vector so it stays crisp at any size. Google Authenticator and Microsoft
 * Authenticator's icons are much harder to redraw cleanly by hand — those
 * two are plain image files instead, see authenticator-apps/ next to this
 * file's siblings in assets/auth/. Shown only so a guardian can recognize
 * these apps on their phone's app store, not as an endorsement from Twilio,
 * Google or Microsoft.
 */
export function IconAuthyLogo(props: SVGProps<SVGSVGElement>) {
  return (
